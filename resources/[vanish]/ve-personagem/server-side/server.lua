RegisterServerEvent("ve-personagem:finishedIntro")
RegisterServerEvent("ve-personagem:finishedCharacter")
...
function processSpawnController(source,statusSent,user_id)
	...
	elseif statusSent == 1 or statusSent == 0 then
		userlogin[user_id] = true
		TriggerClientEvent("ve-personagem:characterCreate",source)
	end
end

AddEventHandler("ve-personagem:finishedIntro", function()
	--print("finishedIntro")
	-- setUData spawnController = 1
	TriggerClientEvent("ve-personagem:characterCreate", source)
end)

AddEventHandler("ve-personagem:finishedCharacter", function(characterNome, characterSobrenome, characterAge, currentCharacterMode)
	local source = source
...
function doSpawnPlayer(source,user_id,firstspawn)
	TriggerClientEvent("ve-personagem:normalSpawn",source,firstspawn)
	TriggerEvent("ve-barbershop:init",user_id)
end