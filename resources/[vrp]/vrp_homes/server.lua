...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookbaucasas = "https://discord.com/api/webhooks/867458876671655966/JOYgovVvS1__3Qgil_5Jv0jJcy0Vq_F2bnkTfq5MkzccqRQFibogTuRxx6zWH5mKTBul"
...
-----------------------------------------------------MANSAO------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
	["MS01"] = { 15000000,2,1500 },
	["MS02"] = { 15000000,2,1500 },
	["MS03"] = { 15000000,10,1500 },
	["MS04"] = { 15000000,5,1500 },
	["MS05"] = { 15000000,2,1500 },
	["MS06"] = { 15000000,2,1500 },
	["MS07"] = { 15000000,2,1500 },
	["MS08"] = { 15000000,2,1500 },
	["MS09"] = { 15000000,2,1500 },
-----------------------------------------------------------------------------------------------------------------------------------------
-------------------------------------------------------------SANDYSHORE------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
	--["SS01"] = { 15000000,2,1000 },
-----------------------------------------------------------------------------------------------------------------------------------------
-------------------------------------------------------------SANDYSHORE------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
	["FZ01"] = { 15000000,2,1500 },
	-----------------------------------------------------------------------------------------------------------------------------------------
-------------------------------------------------------------MORGANA------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
	["MO01"] = { 15000000,10,1500 },
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIÁVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1000)
		for k,v in pairs(processando) do
			if v > 0 then
				processando[k] = v - 1
				if v == 0 then
					processando[k] = nil
...
function src.storeItem(homeName,itemName,amount)
	...
	if itemName then
		local user_id = vRP.getUserId(source)
		local identity = vRP.getUserIdentity(user_id)
		if user_id and processando[parseInt(user_id)] == 0 or not processando[parseInt(user_id)] then
			processando[parseInt(user_id)] = 2
			...
			TriggerClientEvent("Notify",source,"negado","O item está indo pro báu, aguarde um segundo.",8000)
		end
	end
	return false
end
...
function src.takeItem(homeName,itemName,amount)
	...
	if itemName then
		...
		if user_id and processando[parseInt(user_id)] == 0 or not processando[parseInt(user_id)] then
			...
			TriggerClientEvent("Notify",source,"negado","O item está indo pro inventario, aguarde um segundo.",8000)
		end
	end
	return false
end
