...
vRPts = Tunnel.getInterface("vrp_tattoos")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local cam = nil
...
function cRP.setTattoos(data)
	if data then
		custom = data
		canStartTread = true
		-- if (IsPedModel(PlayerPedId(),"mp_m_freemode_01") or IsPedModel(PlayerPedId(),"mp_f_freemode_01")) and custom then
		-- 	Citizen.Wait(5000)
		-- 	setTattoos()
		-- end
	end
...
AddEventHandler("vrp_tattoos:setTattoos",function()
	atualTattoo = custom
...
RegisterNUICallback("close",function(data,cb)
	RenderScriptCams(false,true,250,1,0)
	vRPts.updateTattoo(atualTattoo)
...
RegisterCommand('opent', function(source, args, rawCmd)
    TriggerEvent("hudDeactived")
	old_custom = vRP.getCustomPlayer(source)
	openTattooShop()
end)
