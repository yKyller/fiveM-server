...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookbankrobbery = "https://discord.com/api/webhooks/867272575628738570/DE9c7LXFHCkEsjRO1BZFXJf2LvSN44A--gpKzlSi6wyqWDejvMdWv_clLtJe45WyoNj4"
...
function vRPrb.updateCops(message)
    ...
    local user_id = vRP.getUserId(source)
    if user_id then
        TriggerClientEvent('chatMessage', -1, "[LASTNEWS BANCO CENTRAL]", {64,64,255}, message)
    end
end
...
AddEventHandler('vrp_roubobanco:lootSafe', function(bank, safe)
	...
	vRPclient._notify(src,"Você encontrou ~g~ RR$".. randommoney .."!")
