...
function SetVehicleNitroPurgeEnabled(vehicle, enabled)
  ...
  if enabled then
    ...
    for i=0,3 do
      local leftPurge = CreateVehiclePurgeSpray(vehicle, off.x - 0.5, off.y + 0.05, off.z, 40.0, -20.0, 0.0, 0.8)
      local rightPurge = CreateVehiclePurgeSpray(vehicle, off.x + 0.5, off.y + 0.05, off.z, 40.0, 20.0, 0.0, 0.8)
      -- local leftPurge = CreateVehiclePurgeSpray(vehicle, GetEntityBoneIndexByName(vehicle, "wheel_lf"), 0.8, "l")
      -- local rightPurge = CreateVehiclePurgeSpray(vehicle, GetEntityBoneIndexByName(vehicle, "wheel_rf"), 0.8, "r")
