...
AddEventHandler('bank:pagarmulta', function(amount)
  ...
  if amount < 1000 then TriggerClientEvent("Notify", source, 'aviso',"Aviso","Você só pode pagar multas acima de <b>$1000</b> dolares") return end
  ...
  if vRP.tryFullPayment(user_id,rounded) then
    ...
    TriggerClientEvent("Notify", source, 'sucesso',"Sucesso","Voce pagou <b>$"..rounded.." em multas </b>")
  else
    TriggerClientEvent("Notify", source, 'negado',"Negado","<b>Dinheiro Insuficiente </b>")
  end
end)
    ...
    if amount and type(amount) == "number" then
      ...
      if (rounded > 0) then
        ...
        if (rounded <= wallet) then
          ...
          TriggerClientEvent("Notify", source, 'sucesso',"Sucesso","Você acabou de depositar <b>$" ..addComma(amount).."</b>")
        else
          TriggerClientEvent("Notify", source, 'negado',"Negado","<b>Dinheiro Insuficiente </b>")
        end
      end
  ...
  if user_id then
    if amount and type(amount) == "number" then
      ...
      if (rounded <= bankbalance) then
        ...
        -- Salva o extrato
        TriggerClientEvent("Notify", source, 'sucesso',"Sucesso","Você acabou de sacar <b>$" ..rounded.."</b>")
      else
        TriggerClientEvent("Notify", source, 'negado',"Negado","<b>Dinheiro Insuficiente </b>")
      end
...
AddEventHandler('bank:quickCash', function()
  ...
  if (bankbalance >= quantia) then
    ...
    TriggerClientEvent("Notify", source, 'sucesso',"Sucesso","Você acabou de sacar <b>$1000!</b>")
    ...
    if amount and type(amount) == "number" then
      ...
      if (rounded > 0) then
        ...
        if (rounded <= bankbalance) then
          ...
          TriggerClientEvent("Notify", source, 'sucesso',"Sucesso","Você transferiu <b>$"..rounded.."</b> para o <b>ID: "..nuser_id.."</b>")
        else
          TriggerClientEvent("Notify", source, 'negado',"Negado","<b>Dinheiro Insuficiente </b>")
        end
      else
        TriggerClientEvent("Notify", source, 'negado',"Negado","<b>Você não pode transferir esse valor!</b>")
      end
    end
  else
    TriggerClientEvent("Notify", source, 'aviso',"Aviso","<b>Impossivel transferir para você mesmo!</b>")
  end
end)