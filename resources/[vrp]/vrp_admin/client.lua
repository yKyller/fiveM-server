...
--[ TUNING ]-----------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("newplate")
AddEventHandler("newplate",function(placa)
	local ped = PlayerPedId()
...
--[ TP CORRIDAS ]------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent('tpcorrida')
AddEventHandler('tpcorrida',function(x,y,z)
	local ped = PlayerPedId()
...
-- REMOVER PROCURADO
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent('rprocurado')
AddEventHandler('rprocurado',function(source)
	if vRP.getStandBY(source) > 0 then
		vRP.resetStandBY(source) 
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("vehtuning2",function()
	...
	if IsEntityAVehicle(vehicle) then
        ...
        SetVehicleTyresCanBurst(vehicle,false)
        --SetVehicleNumberPlateText(vehicle,"TDB")
        --SetVehicleNumberPlateTextIndex(vehicle,5)
...
RegisterCommand("setinicio",function(source,args)
...
RegisterCommand("tpinicio",function(source,args)
    local ped = PlayerPedId()
	local veh = GetVehiclePedIsUsing(ped)
	if IsPedInAnyVehicle(ped) then
		ped = veh
	...
	SetEntityCoordsNoOffset(ped,iniciox,inicioy,inicioz,0,0,1)
end)